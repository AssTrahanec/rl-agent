...
def compute_metrics(returns: np.ndarray) -> dict:
    """Compute financial metrics from array of daily returns.

    Args:
        returns: 1D array of daily simple returns (e.g., 0.01 = 1% gain).

    Returns:
        Dict with sharpe_ratio, sortino_ratio, max_drawdown, calmar_ratio, total_return.
    """
    ...
    sharpe_ratio = float(
        (mean_r / std_r) * np.sqrt(TRADING_DAYS_PER_YEAR)
    ) if std_r > 0 else 0.0

    downside = returns[returns < 0]
    downside_std = float(np.std(downside, ddof=1)) if len(downside) > 1 else 1.0
    sortino_ratio = float(
        (mean_r / downside_std) * np.sqrt(TRADING_DAYS_PER_YEAR)
