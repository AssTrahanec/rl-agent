...
**Зачем нужен именно средний квадрат `B`, а не только среднее `A`.** Среднее говорит, *где*
доходности, но не *как сильно* они скачут. Два агента с одинаковым средним = 0:

| Агент | Доходности по барам | Среднее `A` | Риск |
|---|---|---|---|
| Спокойный | `0, 0, 0, 0` | 0 | нулевой |
| Дёрганый | `+10, −10, +10, −10` | 0 | огромный |

По `A` их не отличить. А `B` (среднее квадратов): у спокойного `0`, у дёрганого `100` — вот он и
ловит разброс. Квадрат убирает знаки (`+10` и `−10` → оба `100`) и усиливает крупные движения.

**Будут ли `A²` и `B` вообще различаться?** Да, почти всегда. Возведение в квадрат нелинейно,
поэтому «усреднить-потом-квадрат» (`A²`) ≠ «квадрат-потом-усреднить» (`B`). Равны они **только если
все доходности одинаковые** (нулевой разброс):

| Доходности | `A` | `A²` | `B` | дисперсия `B − A²` |
|---|---|---|---|---|
| `4, 4` (нет разброса) | 4 | 16 | 16 | **0** (равны) |
| `3, 5` | 4 | 16 | 17 | 1 |
| `0, 8` | 4 | 16 | 32 | 16 |

(`B ≥ A²` — всегда; равенство только при одинаковых значениях.) В реальном рынке цена каждый бар
двигается по-разному, поэтому `B − A²` всегда > 0 — это и есть риск, который DSR делит в знаменателе.
В самом начале `A = B = 0`, и работает защита `if denom < 1e-12: reward = R_t`.
