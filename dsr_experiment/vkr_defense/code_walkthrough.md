...
### 3.1. Почему log-доходность (а не обычные проценты)

`log_return = ln(price_next / price_curr)` вместо `price_next/price_curr − 1`. Причины:
- **Складываются во времени.** Сумма лог-доходностей по барам = лог-доходность за весь период
  (`ln(p₂/p₀) = ln(p₂/p₁) + ln(p₁/p₀)`). Обычные проценты так не складываются — их надо
  перемножать. Для DSR это важно: бегущие среднее/дисперсия по лог-доходностям ведут себя
  корректно и чисто аннуализируются.
- **Симметрия прибыли и убытка.** В логах +x и −x точно компенсируются. В процентах +10%, потом
  −10% не возвращают к старту: `1.1 × 0.9 = 0.99`. Для меры риска (Шарп) симметрия важна.
- **Стандарт в финансах.** Лог-доходности ближе к нормальному распределению — статистики
  (среднее, std, Шарп) на них работают «как в учебнике».

`R_t = log_return · allocation` — доходность портфеля при доле `allocation` в BTC (для долей 0/1 —
либо весь рост/падение, либо ноль). В **бэктесте** лог обратно переводят в обычную доходность
`exp(R_t) − 1`, чтобы считать реальную equity-кривую (деньги компаундятся в процентах).

### 3.2. Награда «на пальцах» (DSR)

Цель — максимизировать **не прибыль, а коэффициент Шарпа** (доходность / разброс). Иначе агент делал
бы рискованные ставки. Но Шарп считается по всему периоду, а RL нужна награда на каждом шаге.

**DSR** даёт пошаговую награду = «**насколько этот бар улучшил мой текущий Шарп**». Сумма таких
наград за эпизод ≈ итоговый Шарп → максимизируя награду, агент максимизирует Шарп.

- доходность **выше** бегущего среднего → награда **+**;
- доходность ниже / убыток → **−**;
- сильный скачок (хоть вверх) раздувает разброс → награда **приглушается** (Шарп не любит
  волатильность); делёж на `(разброс)^1.5` усиливает этот эффект.

### 3.3. Что такое `A` и `B`

Два числа, которые среда хранит и чуть обновляет каждый шаг (старт — 0):
- **`A`** = бегущее среднее доходностей `R_t` (≈ μ, средняя прибыль);
- **`B`** = бегущее среднее **квадратов** `R_t²` (второй момент).

Из них собирается риск и Шарп: **дисперсия = `B − A²`**, `Sharpe = A / √(B − A²)`. Двух чисел
достаточно, чтобы знать и среднее, и разброс — **не храня всю историю**. Обновление —
экспоненциальное скользящее среднее с η = 0.01 (помнит ~100 последних баров):
```python
A += 0.01·(R_t − A);   B += 0.01·(R_t² − B)
```
В формуле награды `dA = R_t − A`, `dB = R_t² − B` — «на сколько свежий бар отличается от среднего»,
а `B − A²` в знаменателе — та самая дисперсия (риск), на которую делят.
