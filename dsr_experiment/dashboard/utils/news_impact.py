...
def inject_news_features(features, feature_columns, sentiment, emb_64):
    """Return a copy of `features` with one news article injected into every row.
    ...
    The news is written into all rows so the agent sees a consistent news
    environment across its observation window — this matches the training
    distribution, where news was present in nearly every bar. Injecting into a
    single bar leaves the agent in an unseen state and yields reactions
    inconsistent with the news sentiment.
